...
if ($s == "edit")
	...
	//query
	$qx = mysqli_query($koneksi, "SELECT * FROM m_kuisioner_dosen ".
				"WHERE kd = '$kdx'");
...
if ($_POST['btnSMP'])
	...
	if (empty($e_nama))
		...
		{ ///cek
		$qcc = mysqli_query($koneksi, "SELECT * FROM m_kuisioner_dosen ".
								"WHERE e_nama = '$e_nama'");
		...
		if ($tcc != 0)
			...
			if (empty($s))
				{
				//query
				mysqli_query($koneksi, "INSERT INTO m_kuisioner_dosen(kd, nama, postdate) VALUES ".
								"('$x', '$e_nama', '$today')");
			...
			//jika update
				...
				{
				//query
				mysqli_query($koneksi, "UPDATE m_kuisioner_dosen SET nama = '$e_nama', ".
								"postdate = '$today' ".
								"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_kuisioner_dosen ".
						"WHERE kd = '$kd'");
		}
...
if (empty($s))
	...
	if ($total != 0)
		...
		do
			...
			if (!empty($dkd))
				...
				//nilai
				$nil_kolom = "jawaban_1";
				$qku = mysqli_query($koneksi, "SELECT * FROM mahasiswa_kuisioner_dosen ".
	...
	//query
	$q = mysqli_query($koneksi, "SELECT * FROM mahasiswa_kuisioner_dosen ".
