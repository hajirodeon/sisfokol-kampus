...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//hapus
		mysqli_query($koneksi, "DELETE FROM mahasiswa_sp ".
						"WHERE kd_mhs = '$kdix' ".
...
if ($_POST['btnSMPx'])
	...
	//cek, 
	$qc = mysqli_query($koneksi, "SELECT * FROM mahasiswa_sp ".
	...
	if ($tc != 0)
		...
		{
		//query
		mysqli_query($koneksi, "INSERT INTO mahasiswa_sp(kd, kd_mhs, kd_tapel, ket, tgl, postdate) VALUES ".
...
if ($_POST['btnSMP2'])
	...
	for($i=1;$i<=$total;$i++)
		...
		//query
		mysqli_query($koneksi, "UPDATE mahasiswa_sp SET ket = '$absxx' ".
						"WHERE kd_mhs = '$kdx' ".
						"AND kd_tapel = '$tapelkd'");
		}
...
*/
	...
	if ($count != 0)
		...
		do
			...
			//detail e
			$qku = mysqli_query($koneksi, "SELECT m_mahasiswa.*, mahasiswa_sp.* ".
