...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//hapus
		mysqli_query($koneksi, "DELETE FROM mahasiswa_cuti ".
						"WHERE kd_mhs = '$kdix' ".
						"AND kd_smt = '$smtkd'");
		}
...
if ($_POST['btnSMPx'])
	...
	//cek, 
	$qc = mysqli_query($koneksi, "SELECT * FROM mahasiswa_cuti ".
						"WHERE kd_smt = '$smtkd' ".
						"AND kd_mhs = '$siswa'");
	...
	if ($tc != 0)
		...
		{
		//query
		mysqli_query($koneksi, "INSERT INTO mahasiswa_cuti(kd, kd_mhs, kd_smt, ".
...
if ($_POST['btnSMP2'])
	...
	for($i=1;$i<=$total;$i++)
		...
		//query
		mysqli_query($koneksi, "UPDATE mahasiswa_cuti SET status = '$absx', ".
						"ket = '$absxx' ".
						"WHERE kd_mhs = '$kdx' ".
						"AND kd_smt = '$smtkd'");
		}
...
if (empty($progdi))
	...
	if ($count != 0)
		...
		do
			...
			//detail
			$qku = mysqli_query($koneksi, "SELECT m_mahasiswa.*, m_mahasiswa.kd AS mskd, mahasiswa_kelas.*, mahasiswa_cuti.* ".
									"FROM m_mahasiswa, mahasiswa_kelas, mahasiswa_cuti ".
									"WHERE mahasiswa_kelas.kd_mahasiswa = m_mahasiswa.kd ".
									"AND mahasiswa_cuti.kd_mhs = m_mahasiswa.kd ".
									"AND mahasiswa_kelas.kd_progdi = '$progdi' ".
