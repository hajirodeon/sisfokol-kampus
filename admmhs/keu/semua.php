...
if (empty($smtkd))
	...
	//daftar jenis keu.
	$qjku = mysqli_query($koneksi, "SELECT * FROM m_keu_jenis ".
				"ORDER BY nama ASC");
	$rjku = mysqli_fetch_assoc($qjku);
	...
	while ($rjku = mysqli_fetch_assoc($qjku));
	...
	$qpkl = mysqli_query($koneksi, "SELECT SUM(biaya) AS total ".
				"FROM m_keu ".
	...
	//yang telah dibayar, SS
	$qccx2 = mysqli_query($koneksi, "SELECT SUM(nilai) AS nilai FROM mahasiswa_keu ".
